<template>
  <section class="krules">
    ...
    <div v-if="canApplyDefaults && templates.length" class="krules__defaults">
      <label>Modelo de regras
        <select v-model="selectedTemplate"><option v-for="template in templates" :key="template.key" :value="template.key">{{ template.name }}</option></select>
      </label>
      <button type="button" :disabled="saving || !selectedTemplate" @click="applyDefaults">Aplicar regras padrão</button>
    </div>
    <p v-if="error && !form.open" class="krules-modal__error">{{ error }}</p>
...
const props = defineProps({ station: { type: Object, required: true }, templates: { type: Array, default: () => [] } });
const emit = defineEmits(["saved"]);
const rules = ref([]);
const saving = ref(false);
const error = ref("");
const selectedTemplate = ref("");
const canApplyDefaults = computed(() => !rules.value.length ||
  (rules.value.length === 1 && rules.value[0].id === "include-all"));
...
watch(() => props.station, (station) => { rules.value = clone(station.rules || []); }, { immediate: true, deep: true });
watch([() => props.station.id, () => props.templates], () => {
  selectedTemplate.value = props.templates.find((template) => template.sectors?.some(
    (sector) => props.station.sectors?.includes(sector),
  ))?.key || props.templates[0]?.key || "";
}, { immediate: true });
...
function openRule(rule, index = null) { error.value = ""; Object.assign(form, rule ? { ...clone(rule), open: true, index } : { ...blank(), open: true }); }

async function applyDefaults() {
  saving.value = true; error.value = "";
  try {
    const { data } = await api.post(`/kitchen/stations/${props.station.id}/apply-template-rules/`, {
      template: selectedTemplate.value,
    });
    rules.value = clone(data.rules || []);
    emit("saved", data);
...
async function persist(next) {
  ...
  try {
    ...
    rules.value = clone(data.rules || []); emit("saved", data);
  } catch (err) { error.value = normalizeApiError(err).message; }
  finally { saving.value = false; }
}
async function saveRule() {
  const rule = { id: form.id || `rule-${Date.now()}`, name: form.name.trim(), action: form.action, match: form.match, target_column: form.action === "move" ? form.target_column : null, enabled: form.enabled, conditions: clone(form.conditions) };
...
function toggle(index) { const next = clone(rules.value); next[index].enabled = !next[index].enabled; persist(next); }
function reorder(index, delta) { const next = clone(rules.value); [next[index], next[index + delta]] = [next[index + delta], next[index]]; persist(next); }
</script>

<style scoped>
.krules__defaults{display:flex;align-items:end;gap:8px;flex-wrap:wrap}.krules__defaults label{display:flex;flex-direction:column;gap:4px;color:var(--text-body);font-size:12px}.krules__defaults select,.krules__defaults button{height:34px;padding:0 10px;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--surface-card);color:var(--text-body);cursor:pointer}
