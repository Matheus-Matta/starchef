...
backend/
  ...
  apps/
    ...
    images/               acervo central de imagens e galerias de produto
...
Relatórios (`apps/reports`): `SalesReportView` e derivadas por seção. A seção `orders` (`/reports/orders/`) traz, além dos agregados, o detalhe dos cancelamentos (`apps/reports/order_cancellations.py`): `cancelled_orders` (quem cancelou, quem liberou, `cancel_authorization` = própria/senha do caixa/usuário autorizado/dentro da carência, motivo, valor, minutos aberto), `voided_items` (desistência × cortesia, antes/depois da cozinha), `cancelled_by_user`, `cancelled_by_hour`, `cancelled_by_authorization`, KPIs e CSV. Esses dados vêm das colunas `Order.cancelled_at/cancelled_by/cancel_authorized_by/cancel_authorization` e `OrderItem.voided_at/voided_by`, preenchidas em `cancel_order`/`void_order_item`/`comp_order_item` e retroalimentadas do `AuditLog` pela migração `orders.0008`. O caixa tem relatório próprio em `/reports/cash-movements/` (`apps/reports/cash_movements.py`: gaveta do período, por tipo/caixa/operador/dia, sessões, movimentos, CSV) e o histórico `/cash-movements/` (somente leitura, filtros por período/caixa/sessão/operador/tipo/status, `export=csv`). `GET /cash-register/{id}/statement/` monta o extrato auditável de uma sessão com movimentos, recebimentos, pedidos e itens vinculados, usado pelo detalhamento, impressão e exportação da retaguarda. O escopo dos relatórios é conta + restaurante; a filial não filtra (pedido nasce com `branch=None`).
...
## Ambiente e armazenamento de mídia

### Acervo de imagens dos cadastros

`apps/images` centraliza os arquivos enviados para restaurante e cardápio. O
model `Image` guarda arquivo, nome original, MIME detectado, dimensões, tamanho,
SHA-256 e texto alternativo; `ProductImage` mantém a ordem da galeria. JPG,
JPEG e PNG são aceitos, o conteúdo real é conferido pelo Pillow e SVG é
recusado. Todos os vínculos são restritos à mesma conta.

Os serializers aceitam multipart pelos campos `logo_upload` (restaurante e
categoria), `logo_p_upload`, `photo_uploads` e `photo_remove_ids` (produto). A resposta expõe
`logo_url`, `logo_p`, `logo_p_id` e `photo_list`. A variação recebe
`logo_image`, que só pode apontar para uma imagem presente em `photo_list` do
produto. A variação sem vínculo fica sem imagem; ela não herda a principal do
produto. Remover uma foto também limpa seu uso em variações e, se ela era a
principal, promove a primeira foto restante. Os campos antigos `Restaurant.logo` e `Product.image` continuam como
fallback de leitura e a migration importa os arquivos já cadastrados.

### Uma variável define o ambiente

`DJANGO_ENV` é a única necessária. Ela escolhe o módulo de settings, liga/desliga
o `DEBUG`, decide o banco (SQLite em desenvolvimento, Postgres em produção) e o
`Secure` dos cookies. `DJANGO_DEBUG` e `DJANGO_SETTINGS_MODULE` não precisam mais
ser mantidos em sincronia — manter três variáveis dizendo a mesma coisa era a
origem de um bug silencioso: ambiente "de desenvolvimento" com `DJANGO_DEBUG=False`
ligava cookies `Secure` e o login parava de funcionar em HTTP, sem mensagem.

Valores aceitos: `development`/`dev`/`local` e `production`/`prod`/`staging`. Um
nome desconhecido é tratado como **produção** — errar para o lado seguro.

Precedência (`backend/config/env.py`):

1. `DJANGO_SETTINGS_MODULE` **exportado no processo** (docker-compose, CI, `--settings`);
2. `DJANGO_ENV` — no processo, depois no `.env`;
3. `DJANGO_DEBUG` — idem, só por compatibilidade;
4. nada informado: desenvolvimento.
...
serviço só, consultado por `STORAGES["default"]`:

| `AWS_STORAGE_BUCKET_NAME` | Destino |
| --- | --- |
| vazio | disco local (`MEDIA_ROOT`), servido em `MEDIA_URL` |
| preenchido | S3/R2 via `django-storages` |

**Credencial faltando não derruba o processo.** O servidor sobe, o admin abre, a
API serve leitura e o healthcheck passa. O erro aparece só na **gravação**, como
`503` com `code=media_storage_unavailable` e o nome das variáveis que faltam — não
como um `NoCredentialsError` do boto3. A razão é operacional: um deploy com uma
variável esquecida deve degradar o upload de imagem, não tirar o sistema do ar.

`manage.py check` emite o aviso `core.W001` quando o S3 está configurado pela
metade — um **Warning**, nunca um Error, para o `runserver` continuar subindo.
...
## Menus (estilo Shopify)

`menu.Menu` deixou de ser só "curadoria de catálogo": virou uma **coleção
nomeada** que os blocos do site consomem. O mesmo modelo alimenta quatro coisas
que exigiriam quatro cadastros — a barra de navegação do cabeçalho (com
submenus), o carrossel de banners, a vitrine de categorias/produtos e a
curadoria de canal que o `MenuSite.catalog` já usava.

**Handle.** `slug` é único por **conta**, não globalmente. Antes era
`unique=True`: a primeira conta que criasse um menu "principal" travava o nome
para toda a plataforma.

**Tipos de item** (`MenuItem.item_type`) — o tipo diz o que a entrada *é*; o
destino diz para onde ela *leva*, e nem sempre coincidem:

| tipo | mostra | leva para |
| --- | --- | --- |
| `product` | o produto (nome, foto, preço) | a página do produto |
| `category` | a categoria | a listagem da categoria |
| `image` | a imagem enviada no item | o `url` informado |
| `custom` | só o título | o `url` informado |

`image` num item de produto/categoria **substitui** a foto padrão — é assim que
uma arte de campanha entra no carrossel sem trocar a foto do cardápio.

**Aninhamento.** `parent` monta os submenus da navegação, com teto de 3 níveis
(o mesmo do Shopify). O serializer recusa ciclo, pai de outro menu e o quarto
nível.

**Origens** (`Menu.source`) — de onde saem os itens:

| origem | resolve para |
| --- | --- |
| `manual` | os itens cadastrados, na ordem escolhida |
| `all_categories` | todas as categorias ativas |
| `category_products` | os produtos de `source_category` |
| `best_sellers` | mais vendidos (soma de `OrderItem.quantity`) |
| `promotions` | produtos com preço promocional |

As dinâmicas respondem sozinhas: criar uma categoria já a coloca no menu de
categorias, sem ninguém editá-lo.

**Menus padrão.** `menu/services/default_menus.py` cria quatro no
provisionamento do site (`categorias`, `destaques`, `ofertas`,
`navegacao-principal`) — sem eles, todo bloco que pede um menu ofereceria uma
lista vazia e o cliente teria de entender o conceito antes de ver um carrossel.

**No site.** `menu/services/menu_resolver.py` resolve qualquer menu numa lista
de entradas de forma idêntica, e o payload público leva todas em `menus`,
indexadas por handle. O `GET /api/v1/menu/menus/{id}/resolved/` devolve o mesmo
para o painel — é o único jeito de conferir um menu dinâmico antes de publicar,
já que ele não tem item cadastrado para olhar.

Uma venda **não** invalida o cache do payload de propósito: durante o serviço,
cada item de pedido derrubaria o cardápio inteiro. Um "mais vendidos" com até o
TTL do cache de atraso não incomoda ninguém.
