...
void main() {
  ...
  ApiClient clientWith(http.Client transport) => ApiClient(
  ...
  test('uma queda de rede não apaga a sessão guardada', () async {
    final api = clientWith(
    ...
    // Encerrar a sessão por um tropeço de rede deixaria o terminal sem
    // conseguir entrar de novo até a internet voltar — e o login também exige
    // servidor. O operador perderia o turno por causa de um cabo.
    expect(restored, isNotNull);
  ...
  test('sem Lembrar-me a sessão não é gravada no cofre', () async {
    final api = clientWith(
      MockClient(
        (_) async => http.Response(
          jsonEncode({
    ...
    final repository = AuthRepository(apiClient: api, sessionStore: sessions);
  ...
  test('sem rede não há login: a recusa é explícita', () async {
    // Este PDV só opera conectado. Entrar com uma credencial que ninguém
    // conferiu daria ao operador uma tela inteira de gestos que falham um a
    // um — abrir caixa, consultar produto, registrar venda.
    final api = clientWith(
    ...
    await expectLater(
      ...
      throwsA(
        isA<ApiException>()
            .having((error) => error.isConnectivity, 'conectividade', isTrue)
            .having(
              (error) => error.message,
              'mensagem',
              contains('só opera conectado'),
            ),
      ),
    );
    await api.dispose();
  ...
  test('senha recusada pelo servidor não vira falha de rede', () async {
    final api = clientWith(
      MockClient(
        (_) async => http.Response(
    ...
    await expectLater(
      ...
      throwsA(
        isA<ApiException>()
            ...
            .having((error) => error.isConnectivity, 'conectividade', isFalse),
      ),
    );
    await api.dispose();
  });
