...
import 'package:starchef_pdv_desktop/features/scale/data/scanner_binding_store.dart';
  ...
  group('ScannerBindingStore', () {
    ...
    test('does not allow two slots to reserve the same serial port', () async {
      ...
      await expectLater(
        ...
        throwsA(
          isA<ScannerPortAlreadyBound>()
              .having((error) => error.portName, 'porta', 'COM7')
              .having(
                (error) => error.boundSlot,
                'slot que já tem a porta',
                'restaurant-1:scale-1',
              ),
        ),
      );
