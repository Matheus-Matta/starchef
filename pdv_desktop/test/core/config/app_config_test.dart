...
import 'package:starchef_pdv_desktop/core/config/app_config.dart';
...
''');
  ...
  test('usa API_URL compartilhada pelo projeto', () async {
    ...
    // Aqui o endereço veio do `.env`, então é ele que vale — inclusive quando
    // aponta para produção. O padrão do build não entra nesta conta.
    expect(config.apiBaseUrl, 'https://api.starchef.com.br/api/v1');
    expect(config.usedFallbackApiUrl, isFalse);
  });

  test('sem configuração usa o endereço padrão do build', () async {
    final originalDirectory = Directory.current;
    final directory = await Directory.systemTemp.createTemp(
    ...
    // Quem manda é o que o operador cadastrou na engrenagem do login; isto
    // aqui é só o que sobra quando ninguém cadastrou nada.
  ...
  test('override manual inválido é ignorado e usa o padrão do build', () async {
    final originalDirectory = Directory.current;
    final directory = await Directory.systemTemp.createTemp(
