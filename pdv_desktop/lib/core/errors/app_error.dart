  ...
  factory AppError.fromApi(
    ...
    if (exception.isConnectivity) {
      return AppError(
        ...
        // Este PDV só opera conectado: sem rede, a operação NÃO ACONTECEU —
        // não ficou guardada em lugar nenhum. Chamar isso de aviso faria o
        // operador seguir para a próxima venda achando que esta entrou.
        severity: AppErrorSeverity.failure,
        recommendedAction:
            'Nada foi registrado. Restabeleça a conexão e repita a operação.',
    ...
    // servidor nenhum: é uma regra do próprio PDV, validada na tela ("somente
    // dinheiro pode ter valor recebido maior que o restante", por exemplo).
    // Chamar isso de "Servidor indisponível" e mandar "verifique a rede" manda
    // o operador atrás de um problema que não existe — e o desvia da correção
    // que estava a um clique.
    if (status == null) {
      return AppError(
