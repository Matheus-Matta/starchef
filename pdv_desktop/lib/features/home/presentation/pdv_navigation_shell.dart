...
class PdvSidebar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final destinations = <_SidebarEntry>[
      const _SidebarEntry(
        destination: PdvDestination.sale,
        label: 'Venda',
      ...
      const _SidebarEntry(
        destination: PdvDestination.commands,
        label: 'Comandas',
...
class _PdvConnectionBadgeState extends State<PdvConnectionBadge> {
  late NetworkStatus _shown = widget.status;
...
class _ConnectionBadgeView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    final offline = status.phase == NetworkPhase.offline;
    // Sem fila de saída, o indicador não tem número para mostrar: ou o
    // servidor responde, ou não responde. Um contador aqui só existiria se
    // houvesse trabalho represado neste terminal — e não há.
    ...
    return Tooltip(
      message: offline
          ? 'Sem conexão com a Retaguarda. Este terminal só opera conectado: '
                'nenhuma venda pode ser registrada até a rede voltar.\n\n'
                '${status.lastError ?? ''}'.trimRight()
          : status.phase == NetworkPhase.degraded
          ? 'O servidor está respondendo com dificuldade.\n\n'
                '${status.lastError ?? ''}'.trimRight()
