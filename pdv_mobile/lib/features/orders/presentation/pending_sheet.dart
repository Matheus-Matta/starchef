...
/// Tudo que este aparelho ainda deve ao backend, num lugar só.
///
...
/// backend do que ainda era só uma intenção deste celular. Agora a lista de
...
Future<void> showPendingSheet(BuildContext context, BackendGateway gateway) =>
    showAppSheet<void>(
      context,
      ...
      children: [
        ...
        if (pending.isEmpty && failed.isEmpty)
          const AppEmptyState(
            ...
            description: 'Tudo o que você lançou já está no backend.',
          )
        else
          Flexible(
            child: ListView(
              ...
              children: [
                if (pending.isNotEmpty) ...[
                  AppSectionLabel(
                    label: 'A caminho do servidor (${pending.length})',
                ...
                if (failed.isNotEmpty) ...[
                  AppSectionLabel(
                    label: 'Não aceitos pelo servidor (${failed.length})',
