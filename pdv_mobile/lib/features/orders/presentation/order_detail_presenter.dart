...
///
/// **Sem conexão com o backend**, lançar item, cancelar item, enviar
/// para a cozinha e vincular mesa ficam salvos no aparelho (ver
/// [BackendGateway]) e aparecem com um selo "aguardando conexão" até o backend
...
class OrderDetailPresenter extends ChangeNotifier {
  ...
  /// De onde veio o pedido na tela: do backend ou da cópia local.
  ReadOrigin _origin = const ReadOrigin.live();
  ReadOrigin get origin => _origin;

  /// Recebimentos já registrados neste pedido, vistos pelo backend.
  ...
  /// Itens cujo cancelamento o backend ainda não confirmou.
  ...
  /// Executa uma gravação e recarrega o pedido — a versão do backend é
  ...
  ///
  /// O backend responde a uma gravação com o pedido JÁ ATUALIZADO —
  /// é a versão mais nova que existe. Antes essa resposta era descartada e a
  /// tela refazia um `GET` logo em seguida; só que a leitura do backend sai
  ...
  ///
  /// Antes cada toque virava uma ida ao backend que podia falhar
  ...
  ///
  /// Os itens sobem um a um (é assim que o backend e o backend os
    ...
    try {
      for (final draft in pending) {
        try {
          ...
          // Salvo no aparelho: sai quando o backend responder.
          queued++;
        }
      ...
      return queued == 0
          ...
          : 'Sem conexão: $sent de ${pending.length} itens foram entregues. '
                'O resto sai quando o backend responder.';
    } catch (error) {
      return describeFailure(error);
    } finally {
      _working = false;
